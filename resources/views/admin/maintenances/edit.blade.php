...
<div class="card">
    <div class="card-header d-flex justify-content-between align-items-center">
        <h5 class="mb-0">Edit Maintenance #{{ $maintenance->maintenance_id }}</h5>
        <a href="{{ route('admin.maintenances.index') }}" class="btn btn-secondary btn-sm">
            <i class="fas fa-arrow-left"></i> Back
        </a>
    </div>
    <div class="card-body">
        <form action="{{ route('admin.maintenances.update', $maintenance->maintenance_id) }}" method="POST">
            @csrf
            @method('PUT')
            <div class="row">
                    ...
                    <select name="car_id" class="form-select @error('car_id') is-invalid @enderror" required>
                        ...
                        @foreach($cars as $car)
                            <option value="{{ $car->car_id }}"
                                {{ old('car_id', $maintenance->car_id) == $car->car_id ? 'selected' : '' }}>
                ...
                <div class="col-md-6 mb-3">
                    ...
                    <input type="date" name="next_due_date"
                           ...
                           value="{{ old('next_due_date', $maintenance->next_due_date?->format('Y-m-d')) }}">
